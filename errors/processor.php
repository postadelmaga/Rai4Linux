...
class Error_Processor
{
    ...
    public function processReport()
    {
        ...
        $this->_renderPage($this->_getStoreTemplate());
    }

    protected function _getStoreTemplate()
    {
        $report_template = 'report.phtml';
        $_domain = $_SERVER['SERVER_NAME'];
        $hostData = explode('.', $_domain);
        $hostData = array_reverse($hostData);

        if (isset($hostData[0]) && $hostData[0] == 'it') {
            $report_template = 'report_it.phtml';
        } elseif (isset($hostData[0]) && $hostData[0] == 'ru') {
            $report_template = 'report_ru.phtml';
        }
